#define WIN32_LEAN_AND_MEAN
#define STRICT
#include <windows.h>
#include <XLat.h>

unsigned char OEM2KOI8[256]=
{};

unsigned char KOI82OEM[256]=
{};


void XLat_OEM2ANSI(char *From,char *To,int Length) {
	if (Length)
		OemToCharBuffA(From,To,Length);
	else
		OemToCharA(From,To);
}

void XLat_ANSI2OEM(char *From,char *To,int Length) {
	if (Length)
		CharToOemBuffA(From,To,Length);
	else
		CharToOemA(From,To);
}

void XLat_OEM2KOI8(char *From,char *To,int Length) {
	if (!Length) Length=strlen(From);
	for (int I=0;I<Length;I++) To[I]=OEM2KOI8[(unsigned char)From[I]];
}

void XLat_KOI82OEM(char *From,char *To,int Length) {
	if (!Length) Length=strlen(From);
	for (int I=0;I<Length;I++) To[I]=KOI82OEM[(unsigned char)From[I]];
}

void XLat_KOI82ANSI(char *From,char *To,int Length) {
	XLat_KOI82OEM(From,To,Length);
	XLat_OEM2ANSI(From,To,Length);
}

void XLat_ANSI2KOI8(char *From,char *To,int Length) {
	XLat_ANSI2OEM(From,To,Length);
	XLat_OEM2KOI8(From,To,Length);
}

void XLat(char *From,XLat_CodePage FromCP,char *To,XLat_CodePage ToCP,int Length) {
	switch (FromCP) {
	case CP_OEM:
		switch (ToCP) {
		case CP_ANSI:XLat_OEM2ANSI(From,To,Length);break;
		case CP_KOI8:XLat_OEM2KOI8(From,To,Length);break;
		}
		break;
	case CP_ANSI:
		switch (ToCP) {
		case CP_OEM:XLat_ANSI2OEM(From,To,Length);break;
		case CP_KOI8:XLat_ANSI2KOI8(From,To,Length);break;
		}
		break;
	case CP_KOI8:
		switch (ToCP) {
		case CP_OEM:XLat_KOI82OEM(From,To,Length);break;
		case CP_ANSI:XLat_KOI82ANSI(From,To,Length);break;
		}
		break;
	}
}
