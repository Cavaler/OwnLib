...
CFarDialog::CFarDialog(int iX,int iY,const TCHAR *szHelpTopic,FARDIALOGFLAGS dwFlags,bool bUseID):
X1(-1),Y1(-1),X2(iX),Y2(iY),Focused(0),HelpTopic(szHelpTopic),
m_hDlg(NULL),m_pWindowProc(NULL),m_pCWindowProc(NULL),m_lParam(0),
m_dwFlags(dwFlags),m_bUseID(bUseID),m_nCancelID(m_nDefaultCancelID),m_bAutoHotkeys(AutoHotkeys)
{
	StringToGUID(szHelpTopic, m_GUID);
}

CFarDialog::CFarDialog(int iX1,int iY1,int iX2,int iY2,const TCHAR *szHelpTopic,FARDIALOGFLAGS dwFlags,bool bUseID):
...
bool CFarDoubleConverter::FromString(const TCHAR *pszBuffer, double &dValue) {
	return _stscanf(pszBuffer, _T("%lf"), &dValue) > 0;
}
