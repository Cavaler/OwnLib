	...
	#define PERSIST_FILETIME_VARIABLE_(Name, Key) QueryRegIntValue(DECLARE_PERSIST_LOAD, Key "Lo", &Name.dwLowDateTime, 0xE1D58000); QueryRegIntValue(DECLARE_PERSIST_LOAD, Key "Hi", &Name.dwHighDateTime, 0x01A8E79F);
	...
	#define PERSIST_FILETIME_VARIABLE_(Name, Key) SetRegIntValue(DECLARE_PERSIST_SAVE, Key "Lo", Name.dwLowDateTime); SetRegIntValue(DECLARE_PERSIST_SAVE, Key "Hi", Name.dwHighDateTime);
#else
