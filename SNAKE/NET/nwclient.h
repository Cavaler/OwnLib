...
struct Packet {
	unsigned short VersionControl;	 /* 0x0001 */
	unsigned short SequenceNumber;
	unsigned short SenderID;
	 /* 0x8000 - for first request. 0xFFFF - for server; */
...
struct JoinAccept {
	...
	unsigned short YourID;	       /* <0x8000 - client   >0x8000 - watching */
	unsigned short CurRX;
...
struct InitPlayer {
       char	Name[16];
       unsigned short ID;
       unsigned short Status; /* Undefined yet */
       unsigned short Len;
       /*unsigned short X[Len], Y[Len];*/
};
...
struct Eaten {
       unsigned short ID;
       unsigned short NewRX;
       unsigned short NewRY;
};
...
struct PlayerInfo {
       ...
       unsigned short Len;
       unsigned short X[MAX_SNAKE_LEN], Y[MAX_SNAKE_LEN];
} Player[MAX_PLAYERS];

unsigned short MyID,Result=0,len=2;
...
void Finalize() {
 switch (Result) {
  case 0xFF:Result=4;break;
  case 0xFE:Result=1;break;
